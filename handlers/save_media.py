...
async def SaveBatchMediaInChannel(bot: Client, editable: Message, message_ids: list):
    try:
        ...
        share_link = f"https://t.me/{Config.BOT_USERNAME}?start=Sanjana_{str_to_b64(str(SaveMessage.message_id))}"       
        await editable.edit(
            f"**Video Link:\n\n{share_link}**\n\n"
...
async def SaveMediaInChannel(bot: Client, editable: Message, message: Message):
    try:
        ...
        share_link = f"https://t.me/{Config.BOT_USERNAME}?start=Sanjana_{str_to_b64(file_er_id)}"
        await editable.edit(
            f"**Video Link:\n\n{share_link}**\n\n"
            f"**Just Click the link to get your Videos**. **",
            parse_mode="Markdown",
